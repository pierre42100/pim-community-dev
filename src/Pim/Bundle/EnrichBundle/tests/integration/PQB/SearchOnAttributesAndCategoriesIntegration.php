 ...
 *
 * Tests of the filter category used in the UI along with some filters with attributes. The search results need be
...
class SearchOnAttributesAndCategoriesIntegration extends AbstractProductQueryBuilderTestCase
{
     ...
     *      - Select a category set on a sub product model (without including sub categories)
     * should show:
     *      - The product sub product model
     */
    public function withoutIncludingSubCategoriesSelectCategoryOnVariantProductAndFilterOnAttributeSetInProductModel()
    : void {
        $result = $this->executeFilter([
            ['categories', Operators::IN_LIST, ['shoes']],
            ['size', Operators::IN_LIST, ['s']],
